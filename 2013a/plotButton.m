...
count = 0;
htableData = get(Htable, 'Data');
for i = 1:size(htableData,1)
    if htableData{i,1} == 1
        count = 1;
    end
end
...
if count == 0
    ...
    for i = 1:size(htableData,1)
        if htableData{i,1} == 1
            count = count + 1;
            legendNames{count} = [htableData{i,4}, ' @ ', htableData{i,6}, 'K (', num2str(htableData{i,5}),'% O2)'];
    ...
    for i = 1:size(dataTable,2)
        ...
        p = roundsd(i/size(dataTable,2),3);
    ...
    datatabledisplayExtent = get(dataTableDisplay, 'Extent');
    width = datatabledisplayExtent(3)/size(legendNames, 2);
    ...
    function [xUnits, yUnits] = setPlot(hh,dd)
        ...
        for ii = 1:size(dataTable,2)
            ...
            if c == 2  % Both xValues && yValues matched
                showlineValue = get(showLine, 'Value');
                if showlineValue == 1
                    hp = plot(ax, xValues(:,1), yValues(:,1), 'o-')
                ...
                warndlg('Please select experiments with similar properties to plot')
            end
            hold(ax, 'on')
        end
